...
/*
bug on mobile: when moving a node it becomes fixed
...
links.Network.prototype._create = function () {
  ...
  //this.frame.canvas.style.background = "url('img/skies.jpg')";
  ...
  //links.Network.addEventListener(this.frame.canvas, "mousemove", onmousemove); //delete mousemove event by suhuawei
  links.Network.addEventListener(this.frame.canvas, "click", onmousemove); //add click event by suhuawei
...
links.Network.prototype._checkShowPopup = function (x, y) {
  ...
  if (this.popupNode == undefined) {
    ...
    for (var i = nodes.length - 1; i >= 0; i--) { //修改，解决第一个节点无法显示title的问题，by suhuawei 2013-08-30
      var node = nodes[i];
...
links.Network.prototype._selectNodes = function(selection, append) {
  ...
  if (selection.length != this.selection.length) {
    ...
    for (var i = 0, iMax = Math.min(selection.length, this.selection.length); i < iMax; i++) {
  ...
  for (var i = 0; i < rowCount; i++) {
    ...
    for (var col in cols) {
      if (cols.hasOwnProperty(col)) {
        var value = table.getValue(i, cols[col]);
 ...
 * @param {google.visualization.DataTable or Array}   nodesTable  The data containing the nodes.
 */
 ...
 * Note that Object DataTable is defined in google.visualization.DataTable
 * @param {google.visualization.DataTable or Array}      linksTable    The data containing the links.
 */
 ...
 * Note that Object DataTable is defined in google.visualization.DataTable
 * @param {google.visualization.DataTable or Array}   packagesTable    The data containing the packages.
 */
...
links.Network.Node.prototype._drawImage = function (ctx) {
  ...
  if (this.value) {
    ...
    var width = this.radius || this.imageObj.width;
    var height = this.radius * scale || this.imageObj.height;
  }
  else {
    var width = this.imageObj.width;
    var height = this.imageObj.height;
  }
...
links.Network.Link.prototype.setProperties = function(properties) {
  ...
  if (this.style === 'arrow') {
    //this.arrows = [0.5];
    this.arrows = [0.95]; //修改箭头所在位置 by suhuawei
  ...
  switch (this.style) {
    ...
    case 'dashed':          this.draw = this._drawDashedLine ; break;
...
*/
links.Network.Link.prototype._drawDashedLine  = function(ctx) {
